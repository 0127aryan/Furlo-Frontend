    ...
    if (Array.isArray(data) && data.length > 0) {
      ...
      data.forEach((item: SpeciesVerbItem) => {
        if (item.species && item.verb) {
          const key = item.species.toLowerCase().trim().replace(/[\s-]+/g, '_')
...
export function getPetSpecies(pet?: { species?: string | null; pet_type?: string | null; breed?: string | null } | null): string | undefined {
