...
export function AppSidebar() {
  ...
  const navItems = [
    ...
    { label: 'Notifications', href: '/notifications', icon: 'notifications', badge: true },
    { label: 'My Paw Print', href: activePet?.username ? `/p/${activePet.username}` : '/profile', icon: 'pets' },
    ...
    <aside className="w-[240px] h-screen sticky top-0 bg-[#FFFBF7] border-r border-[#EDE8E1] flex flex-col py-6 px-4 shrink-0 font-sans hidden md:flex">
      ...
      <div className="mb-6">
        <Link href="/" className="flex items-center gap-2 group">
          ...
          <span className="text-[22px] font-bold text-[#163328] tracking-tight" style={{ fontFamily: 'Outfit, sans-serif' }}>
            furlo
          </span>
        </Link>
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[14px]" style={{ fontFamily: 'Plus Jakarta Sans, sans-serif' }}>
                {item.label}
              </span>
              {item.badge && <div className="ml-auto w-2 h-2 bg-[#E8843A] rounded-full" />}
            </Link>
          )
        })}
      </nav>
      ...
      <div className="mt-auto pt-4 border-t border-[#EDE8E1]">
        <div className="bg-white rounded-2xl p-3 border border-[#EDE8E1] shadow-[#163328]/5 flex items-center gap-3 cursor-pointer hover:bg-[#f6f9ff] transition-all active:scale-95">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[13px] font-bold text-[#163328] truncate">{petName}</p>
